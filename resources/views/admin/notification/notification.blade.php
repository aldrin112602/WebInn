...
<div class="p-4">
    ...
    <form id="delete-selected-form" action="{{ route('admin.deleteSelected.notifications') }}" method="POST">
        @csrf
        @method('DELETE')
            ...
            <div class="p-4 {{ $notification->is_seen ? 'bg-white' : 'bg-blue-100' }} rounded shadow-md flex items-start justify-between">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <!-- View Button if URL is set -->
                        @if ($notification->url)
                        <button type="button" onclick="window.open('{{ $notification->url }}', '_blank')" class="view-notification-btn bg-blue-500 hover:bg-blue-700 text-white py-1 px-4 rounded mt-2">
                            View
                        </button>
                        @endif
                    </div>
