...
@section('title', 'Account Management | Admin List')
@section('content')
<div>
    <div class="container mx-auto p-4 bg-white">
        ...
        <div class="block md:flex flex-col md:flex-row justify-between items-center mb-4 space-y-2 md:space-y-0 md:space-x-4">
            <div class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4">
                <form id="filterForm" method="GET" action="{{ route('admin.admin_list') }}" class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4">
            ...
            <a href="{{ route('admin.create.admin') }}" class="px-4 py-2 bg-blue-500 text-white rounded-md flex items-center justify-center gap-3"><i class="fas fa-plus"></i> Add Admin</a>
        </div>
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.export.admin') }}" class="px-4 py-2 bg-slate-500 text-white rounded-md flex items-center justify-center gap-3"><i class="fa-solid fa-file-export"></i> Export</a>
            </div>
        </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="py-2 text-center border">
                            <a href="{{ route('admin.edit.admin', $list->id) }}" class="px-2 py-1 bg-blue-500 text-white rounded-md">Edit</a>
                            <button onclick="confirmDelete({{ $list->id }})" class="px-2 py-1 bg-red-500 text-white rounded-md">Delete</button>
                            <form id="delete-form-{{ $list->id }}" action="{{ route('admin.delete.admin', $list->id) }}" method="POST" style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
                        </td>
    ...
    <form id="deleteSelectedForm" action="{{ route('admin.delete.selected.admins') }}" method="POST" style="display: none;">
        @csrf
        @method('DELETE')
