...
@section('content')
    ...
    <div class="p-4 overflow-x-auto">
        ...
        <div class="flex gap-3 items-center justify-between mt-2">
            ...
            <div class="flex items-center justify-start gap-2">
                ...
                <select name="school_year p-1" id="school_year">
                    <option value="{{ $gradingHeaders->year ?? '2023-2024' }}">{{ $gradingHeaders->year ?? '2023-2024' }}
                    </option>
                </select>
            </div>
        </div>
        ...
        <table class="w-full bg-white mt-4">
            <tr class="border">
                <td class="border p-2" rowspan="3">
                    <select name="semester" id="semester" class="border mb-2 w-52 p-0 px-10">
                        ...
                        <option {{ request('semester') == 'First Semester' ? 'selected' : '' }} value="First Semester">First
                            Semester</option>
                        <option {{ request('semester') == 'Second Semester' ? 'selected' : '' }} value="Second Semester">
                    ...
                    <select name="quarter" id="quarter" class="border w-52 p-0 px-10">
                        ...
                        <option {{ request('quarter') == 'First Quarter' ? 'selected' : '' }} value="First Quarter">First
                            Quarter</option>
                        <option {{ request('quarter') == 'Second Quarter' ? 'selected' : '' }} value="Second Quarter">Second
                            Quarter</option>
                        <option {{ request('quarter') == 'Third Quarter' ? 'selected' : '' }} value="Third Quarter">Third
                            Quarter</option>
                        <option {{ request('quarter') == 'Fourth Quarter' ? 'selected' : '' }} value="Fourth Quarter">Fourth
                            Quarter</option>
                    </select>
                </td>
                <td class="border p-2" colspan="13">
                    ...
                    <script>
                        $(document).ready(function () { function n(n, e, a = !1) { var t = new URL(window.location.href); a && t.searchParams.delete("section"), e ? t.searchParams.set(n, e) : t.searchParams.delete(n), window.location.href = t.href } $("#grade").on("change", function () { n("grade", $(this).val()) }), $("#strand").on("change", function () { n("strand", $(this).val(), !0) }), $("#section").on("change", function () { n("section", $(this).val()) }), $("#semester").on("change", function () { n("semester", $(this).val()) }), $("#quarter").on("change", function () { n("quarter", $(this).val()) }), $("#subject").on("change", function () { n("subject", $(this).val()) }), $("#track").on("change", function () { n("track", $(this).val()) }) });
                    </script>
                    ...
                    <div class="flex items-center justify-between mt-2">
                        ...
                        <select name="subject" id="subject" class="border mb-2 w-52 p-0 px-10">
                            ...
                            @foreach ($subjects as $subject)
                                <option {{ request('subject') == $subject->subject ? 'selected' : '' }}
                ...
                <td class="border p-2 text-center" colspan="3" rowspan="3">
                    Quarterly Assessment <br> (<span id="custom_quarterly_assessment_percentage">
                        {{ $percentage_filters[$subj->subject_track ?? '']['quarterly_assessment'] ?? '25%' }} </span> )
                </td>
            ...
            <tr class="border">
                <td class="border p-2" colspan="13">WRITTEN WORK (<span id="custom_written_percentage">
                        {{ $percentage_filters[$subj->subject_track ?? '']['written_work'] ?? '25%' }} </span>) </td>
                <td class="border p-2" colspan="13">PERFORMANCE TASK (<span id="custom_performance_task_percentage">
                        {{ $percentage_filters[$subj->subject_track ?? '']['performance_task'] ?? '50%' }} </span> )</td>
            </tr>
            ...
            <tr class="border">
                ...
                @for ($i = 1; $i <= 10; $i++)
                    <!-- Min: 5, highest: 10 -->
                    <td data-id_written="{{ $i }}" id="highest_possible_score" data-cell-number="{{ $i }}"
                ...
                <td class="border p-2" id="highest_possible_task">
                    {{ $percentage_filters[$subj->subject_track ?? '']['performance_task'] ?? '25%' }}</td>
                <td class="border p-1 cursor-pointer" data-id_quarterly="1" id="quarterly_assessment_total"
                    data-cell-number="1" contenteditable="true">
            ...
            @foreach ($allMaleStudents as $student)
                    <tr>
                        ...
                        @for ($i = 1; $i <= 10; $i++)
                                ...
                                @endphp
                                <td data-for="written_work" data-cell="{{ $i }}" data-user-id="{{ $student->account->id }}"
                                    class="border p-1 cursor-pointer" contenteditable="true">
                                    {{ $writtenGrade }}
                                </td>
                        @endfor
                        ...
                        @for ($i = 1; $i <= 10; $i++)
                                ...
                                @endphp
                                <td data-for="performance_task" data-cell="{{ $i }}" data-user-id="{{ $student->account->id }}"
                                    class="border p-1 cursor-pointer" contenteditable="true">
                                    {{ $taskGrade }}
                                </td>
                        @endfor
                        ...
                        <td data-for="quarterly_assessment" data-cell="11" data-user-id="{{ $student->account->id }}"
                            class="border p-1 cursor-pointer" contenteditable="true">
