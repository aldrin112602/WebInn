...
@section('content')
    <form action="{{ route('teacher.password.otp') }}" method="post"
        ...
        <p class="text-sm text-red-900 mb-4">Enter your email for the verification proccess, we will send 4 digits code to
            your email</p>
        <div class="mb-4">
            <label for="email" class="block text-red-900">Email Address</label>
            <input type="email" id="email" name="email"
                class="form-input w-full rounded border-gray-300 @error('email') border-red-500 @enderror"
                value="{{ old('email') }}">
            @error('email')
        ...
        <div>
            ...
            <p class="text-center mt-5 text-slate-500"><a href="{{ route('teacher.login') }}"
                    class="hover:underline opacity-50 hover:opacity-75 text-sm font-semibold">Back to login</a></p>
        </div>
    </form>
@endsection
