...
@section('content')
    <form action="{{ route('teacher.2fa.verify') }}" method="post"
        ...
        @csrf
        <h2 class="text-2xl font-bold mb-6 mt-5 text-rose-900">Verify Your OTP Code</h2>
        <p class="mb-4 text-gray-500 text-sm">Please enter the 6-digit code sent to your email to complete the verification.
        </p>
        ...
        <div class="mb-6">
            ...
            <div id="otp-container" class="flex justify-between">
                ...
                <input autofocus type="text" maxlength="1"
                    class="otp-box form-input w-12 h-12 text-center rounded-lg border-gray-300 focus:ring-2 focus:ring-rose-900 @error('otp') border-red-500 @enderror">
            ...
            <span id="expiredMessage" class="text-red-500 text-sm hidden">The OTP has expired. Please request a new
                one.</span>
            <script>
        ...
        <div>
            <button type="submit"
                class="w-full bg-rose-900 text-white py-2 rounded-lg font-semibold hover:bg-rose-700 transition duration-200">Verify
                OTP</button>
            <p class="text-center mt-4 text-sm text-gray-500">Didn't receive the code? <a
                    href="{{ route('teacher.2fa.resend') }}" id="resendLink" class="text-blue-600 hover:underline"
                    style="pointer-events: none; opacity: 0.5;">Resend Code</a></p>
        </div>
    </form>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
