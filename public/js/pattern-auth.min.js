...
async function getIsBrowserRestricted() {
    const res = await fetch("/get-user-attempt", {
        method: "POST",
        headers: {
...
async function isBrowserRestricted() {
    let res = await fetch("/user-attempt", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "X-CSRF-TOKEN": $$('meta[name="csrf-token"]').content,
    ...
    if (data.restricted) {
        Swal.fire({
            icon: "error",
            title: "Access Restricted",
            text: `${data.reason}. Please contact the administrator`,
        });
        return true;
    }
    return false;
...
canvas.addEventListener("mousedown", handleStart),
    ...
    submitButton.addEventListener("click", async () => {
        ...
        if (t) {
            let istrue = await getIsBrowserRestricted();
            if (istrue) return;
        }
